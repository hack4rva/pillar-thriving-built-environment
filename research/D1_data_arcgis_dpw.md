...
| **Road Network & Street Centerlines** | "Centerlines" (Richmond GeoHub) [3]; VGIN VBMP RCL [4] | Feature Service / Download [4] [3] | City: Unknown; VGIN: Quarterly [4] | Address, road name, state route number [4] | City attributes/currency unknown; potential conflation issues with VGIN. | Go |
...
| **Construction Permit Activity** | Not found (Possible Socrata or internal DPW) | Unknown | Unknown | Permit ID, type, status, issue/expire, location | May require login, internal API access, or non-GIS source integration. | Watch |
...
* **Timeliness vs. User Expectation**: The CIP dashboard is updated quarterly [1]. Users may expect real-time construction status. **Mitigation**: Prominently display "Data as of [Date]" badges on the UI and link directly to the DPW project page for the latest public meeting notices and updates [2].
