...
While much data is publicly available, some elements require live validation or workarounds:
* **311 Volume Counts:** The exact table name and 12-month volume counts by category on data.richmondgov.com could not be fully verified in this research pass. *Action:* Search the portal live; if found, pull the counts. If not, show the portal search page and commit to backfilling the data.
* **Official Council Districts Layer URL:** The exact REST endpoint for Council Districts was not isolated, though the "Council Lookup App" exists on cor.maps.arcgis.com [5]. *Action:* Locate it via GeoHub search and copy the item URL.
* **Street Cleaning Routes:** Specific layer URLs for street cleaning were not found. *Action:* Cite official PDFs or the Trash Collection Schedule app [13] and explain your proxy approach.
