...
| :--- | :--- | :--- | :--- | :--- | :--- |
| Right-of-way closures / work zones | Verified on Socrata | Richmond Impact Map (aq4i-4gpd) [1] | Weekly [1] | Socrata OData / CSV | Last updated March 31, 2023; 1,559 views, 0 downloads [1]. |
...
| 311 service requests (DPW-related) | Inferred not on portal | RVA 311 Helpful Links [6] | Unknown | Internal CSR | Requires direct export request from the CSR team [6]. |
| Capital improvement projects | Inferred not on portal | N/A | Unknown | Unknown | Likely embedded in budget documents; use Impact Map as proxy. |
| Street cleaning (sweeping) | Unknown | N/A | Unknown | Unknown | No dataset located; likely schedules via PDF or static pages. |
...
### Capital Improvement Projects (Inferred not on portal)
No dedicated CIP dataset was found on Socrata. This data likely resides in budget systems or project-specific pages. For the MVP, teams should proxy "CIP-in-construction" by utilizing the active closures found in the Impact Map [1].
...
### Street Cleaning and Sweeping (Unknown)
No datasets regarding street sweeping schedules or completion records were located. Teams should perform a quick check with DPW Operations; if the data is absent or only available as unstructured PDFs, this category should be deprioritized.
...
The most significant risk to the hackathon projects is silently stale data. For example, the Impact Map indicates weekly updates but has not been refreshed since March 2023 [1]. Teams must embed "last updated" checks and schema validations into their pipelines. Tracking the `last_updated` metadata from Socrata and ArcGIS will prevent applications from displaying outdated closure or paving information as current.
...
* **Sprint 1 (Verification):** Confirm endpoints, schemas, and freshness for the Impact Map [1] and ArcGIS PCI layers [2]. Secure CSV exports for 311 and permit data if possible.
