...
- **The money's origin is largely undocumented.** The corpus never states the mix of
  city bonds, state, and federal dollars behind the CIP budget. The graph models this
  honestly as an explicit `UnknownEntity` source node feeding the CIP fund
  (flow `f:unknown-to-cip`), not as a guess.
- **ARPA**: three projects (Southside Community Center, Lucks Field Community Center,
  the Williamsburg Avenue CSO improvement) are documented as *partially* ARPA-funded —
  but no dollar split is documented anywhere (`q:arpa-portion`).
- **Fall Line Trail**: CVTA and VDOT funding is documented, and so is a **funding gap** —
  the corpus records that a gap exists but not its size (`q:fall-line-gap-size`). This
  is the clearest documented case of a need without money attached.
- **Federal RAISE/TIGER grants** are referenced via USASpending but no specific award
  to Richmond is confirmed in the corpus (`q:raise-tiger-details`, modeled as
...
- **No outcomes are documented for any of the 131 funding flows** (`q:outcome-measurements`).

## Facts vs. proposals vs. hypotheses

The graph separates these by `evidenceStatus`: 756 graph elements are `documented`,
6 `externally_verified`, 16 `proposed` (hackathon solution patterns, not funded
programs), 23 `reported_but_unverified`, 5 `inferred` (each with the inference
stated), 5 `disputed`, 8 explicitly `unknown`. The notable disputed item:
...
app's Open Questions tab; the full narrative is in
